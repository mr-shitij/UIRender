...
public class RenderRadioButton {
    private String declaration = "", body = "", id;

    public RenderRadioButton(String prefix, JSONObject obj, LayoutDecider layoutDecider){
        this.id = Checker.checkForDuplicate(obj, "id", "radiobutton");
        uiBuilder.declaredVariables.add(this.id);
        obj.remove("id");

        if(obj.get("global") == null || !((boolean) obj.get("global"))) {
            this.body = this.body.concat("JRadioButton " + this.id + ";" + "\n");
        }else{
            this.declaration = this.declaration.concat("protected JRadioButton " + this.id + ";" + "\n");
        }
        obj.remove("global");
        ...
        this.body = this.body.concat(this.id + " = new JRadioButton();" + "\n");
