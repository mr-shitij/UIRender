...
public class RenderButton{
    ...
    public RenderButton(String prefix, JSONObject obj, LayoutDecider layoutDecider){
        this.id = Checker.checkForDuplicate(obj, "id", "button");
        uiBuilder.declaredVariables.add(this.id);
        obj.remove("id");


        if(obj.get("global") == null || !((boolean) obj.get("global"))) {
            this.body = this.body.concat("JButton " + this.id + ";" + "\n");
        }else{
            this.declaration = this.declaration.concat("protected JButton " + this.id + ";" + "\n");
        }
        obj.remove("global");

        this.body = this.body.concat(this.id + " = new JButton();" + "\n");
        ...
        for(Object attribute : obj.keySet()){
            switch ((String) attribute){
                ...
                default:
                    ...
                    throw new Error("Invalid Attribute in Button");
            }
        }
