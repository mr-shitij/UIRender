...
public class RenderCheckbox {
    private String declaration = "", body = "", id;

    public RenderCheckbox(String prefix, JSONObject obj, LayoutDecider layoutDecider){
        this.id = Checker.checkForDuplicate(obj, "id", "checkbox");
        uiBuilder.declaredVariables.add(this.id);
        obj.remove("id");

        if(obj.get("global") == null || !((boolean) obj.get("global"))) {
            this.body = this.body.concat("JCheckbox " + this.id + ";" + "\n");
        }else{
            this.declaration = this.declaration.concat("protected JCheckBox " + this.id + ";" + "\n");
        }
        obj.remove("global");
        ...
        this.body = this.body.concat(this.id + " = new JCheckBox();" + "\n");
        ...
        for(Object attribute : obj.keySet()){
            switch ((String) attribute){
                ...
                default:
                    ...
                    throw new Error("Invalid Attribute in CheckBox");
            }
        }
