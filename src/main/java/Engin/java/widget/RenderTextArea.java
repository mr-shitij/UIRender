...
public class RenderTextArea {
    ...
    public RenderTextArea(String prefix, JSONObject obj, LayoutDecider layoutDecider){
        this.id = Checker.checkForDuplicate(obj, "id", "textarea");
        uiBuilder.declaredVariables.add(this.id);
        obj.remove("id");


        if(obj.get("global") == null || !((boolean) obj.get("global"))) {
            this.body = this.body.concat("JTextArea " + this.id + ";" + "\n");
        }else{
            this.declaration = this.declaration.concat("protected JTextArea " + this.id + ";" + "\n");
        }
        obj.remove("global");

        this.body = this.body.concat(this.id + " = new JTextArea();" + "\n");
        ...
        for(Object attribute : obj.keySet()){
            switch ((String) attribute){
                ...
                case "columns":
                    ...
                    break;
                case "rows":
                    this.body =  this.body.concat("\t\t" + this.id + ".setRows(" + obj.get(attribute) + ");" + "\n");
                    break;
                case "font":
                ...
                default:
                    ...
                    throw new Error("Invalid Attribute in TextArea");
            }
        }
